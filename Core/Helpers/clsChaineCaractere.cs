...
namespace Core
{
    public class clsChaineCaractere
    {
        ...
        public string pvgExtraireChaine(string vppValeurChaine, string vppCaractereSeparation, string vppPositionDeDepart)
        {
         ...
         public Boolean pvgValidatePassword(string pwd, int minLength = 8, int numUpper = 2, int numLower = 2, int numNumbers = 2, int numSpecial = 2)
         {
             ...
             //Le mot de passe doit contenir  huit caractères
             if (pwd.Length < minLength)
             {
                 return false;
             }

             // Le mot de passe doit contenir  deux caractères en majuscule
             if (upper.Matches(pwd).Count < numUpper)
             {
                 return false;
             }

             //Le mot de passe doit contenir  deux caractères en minuscule
             if (lower.Matches(pwd).Count >= numLower)
             {
                 return false;
             }

             //Le mot de passe doit contenir  deux chiffres
             if (number.Matches(pwd).Count < numNumbers)
             {
                 return false;
             }

             //Le mot de passe doit contenir deux symboles
        ...
        /// <returns></returns>
         ...
         //--vérifie si la chaine de caractère ne contient que des lettre
         ...
         //--vérifie si la chaine de caractère ne contient que des chiffres
